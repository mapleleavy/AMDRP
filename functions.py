...
class TestbedDataset(InMemoryDataset):
    def __init__(self, root='/tmp', dataset='set', xdf=None, xds=None,
                 xcm=None, xcc=None, xcp=None, y=None,
        ...
        if os.path.isfile(self.processed_paths[0]):
            ...
            self.process(xdf, xds, xcm, xcc, xcp, y, smile_graph)
            self.data, self.slices = torch.load(self.processed_paths[0])
    ...
    def process(self, xdf, xds, xcm, xcc, xcp, y, smile_graph):
        assert (len(xdf) == len(xds) and len(xds) == len(xcm)
                and len(xcm) == len(xcc) and len(xcc) == len(xcp) and len(xcp) == len(y)), "The five lists must be the same length!"
        data_list = []
        data_len = len(xds)
        for i in range(data_len):
            ...
            processedData.methcpg = torch.FloatTensor([methcpg])
