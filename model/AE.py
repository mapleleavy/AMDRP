...
def read_cell_line_methcpg(filename, cell_line_dict):
    f = open(filename, 'r')
    reader = csv.reader(f)
    for i in range(2):
        reader.__next__()
    methcpg = [list() for i in range(len(cell_line_dict))]
    for line in reader:
        if line[0] in cell_line_dict:
            methcpg[cell_line_dict[line[0]]] = line[1:]
    return methcpg
...
def load_data():
    ...
    copynumber = read_cell_line_copynumber('../data/cell_line/461cell-23316dim-copynumber.csv', cell_line_dict)
    # methcpg = read_cell_line_methcpg('../data/cell_line/407cell-69641dim-CpG.csv', cell_line_dict)


    min_max_scaler = preprocessing.MinMaxScaler(feature_range=(0, 1), copy=False)
    copynumber = min_max_scaler.fit_transform(copynumber)
    # methcpg = min_max_scaler.fit_transform(methcpg)
...
def train(train_data, test_data, data_all):
    ...
    best_loss = 1
    res = [[0 for col in range(735)] for row in range(388)]
