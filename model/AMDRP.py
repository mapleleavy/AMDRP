...
from model.model_utils import AFF, MBCA

class AMDRP(torch.nn.Module):
    def __init__(self, n_filters=4, num_features_xd=78, output_dim=128, dropout=0.5, num_heads=8):
        super(AMDRP, self).__init__()
        ...
        self.ds_fc1 = nn.Linear(num_features_xd * 4, 1024)
        self.ds_bn4 = nn.BatchNorm1d(1024)
        self.ds_fc2 = nn.Linear(1024, output_dim)
        ...
        # meth cpg
        self.cp_fc1 = nn.Linear(512, 1024)
        self.cp_bn1 = nn.BatchNorm1d(1024)
        self.cp_fc2 = nn.Linear(1024, 256)
        self.cp_bn2 = nn.BatchNorm1d(256)
        self.cp_fc3 = nn.Linear(256, output_dim)
        self.cp_bn3 = nn.BatchNorm1d(output_dim)

        self.drug_att = AFF(output_dim)
        self.mbca = MBCA(output_dim, num_heads)

        # fusion layers
        self.comb_fc1 = nn.Linear(2 * output_dim, 512)
        self.comb_bn1 = nn.BatchNorm1d(512)
        self.comb_fc2 = nn.Linear(512, 128)
    ...
    def forward(self, data):
        ...
        # Using Attention Mechanisms and MultiHead Cross-Attention
        drug_feat = self.drug_att(x, xdf).unsqueeze(1)
        cell_feat = torch.stack([xcm, xcc, xcp], dim=1)
        drug_cell_pair = self.mbca(drug_feat, cell_feat)

        # fusion
        xfusion = self.comb_fc1(drug_cell_pair)
