...
def train(modeling, train_batch, val_batch, test_batch, lr, epoch_num, cuda_name, i):
    ...
    for epoch in range(epoch_num):
        ...
        ret = [rmse(G, P), mse(G, P), pearson(G, P), spearman(G, P), r2_score(G, P), mae(G, P)]

        G_test, P_test = predict(model, device, test_loader)
        ret_test = [rmse(G_test, P_test), mse(G_test, P_test), mae(G_test, P_test), pearson(G_test, P_test), spearman(G_test, P_test), r2_score(G_test, P_test)]
